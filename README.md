...
- ⚠️ **用 token 跳转时，第一跳一定会停在登录页，按一次刷新（F5）就进去了** —— 这不是 token 无效。实测到的现象是：地址栏先闪一下 `…/?token=…`，然后变成裸 `…/`，再显示 401；而把同一个 token URL 手动粘进地址栏却能直接进。原因是两头都合理、合起来才出问题：harness 的登录 cookie 是 **`SameSite=Strict`**（`dsh-client-connection/lib/index.js`：`…; HttpOnly; SameSite=Strict`），而「打开远端」是从**另一个源**（你这台机器的面板）发起的 **`window.open`，属于跨站发起的导航** —— 浏览器不会在这种导航上附带 Strict cookie。于是"兑换 token → 303 跳回 `/`"那一跳**带不上刚拿到的 cookie**，`/` 就 401 了。地址栏闪过 token 恰恰证明 **token 是被接受的**（那是一次 303）。**刷新是同站导航**，cookie 就带上了，于是进去。手动粘贴也是同站导航，所以直接能进。两个推论：① 配过对的机器走 `pair-app?device=` 不受影响 —— 那个中间页**由远端自己提供**，它再跳 `/` 是同站的；② 进去之后收藏**不带 token 的裸地址**，以后就完全绕开这一跳。
