...
- ⚠️ **用 token 跳转时，每次都会先停在登录页，按一次刷新（F5）才进去 —— 这不是 token 无效，而且这个按钮不会自己好。** 实测到的现象是：地址栏先闪一下 `…/?token=…`，然后变成裸 `…/`，再显示 401；而把同一个 token URL 手动粘进地址栏却能直接进。原因是两头都合理、合起来才出问题：harness 的登录 cookie 是 **`SameSite=Strict`**（`dsh-client-connection/lib/index.js`：`…; HttpOnly; SameSite=Strict`），而「打开远端」是从**另一个源**（你这台机器的面板）发起的 **`window.open`，属于跨站发起的导航** —— 浏览器不会在这种导航上附带 Strict cookie。于是"兑换 token → 303 跳回 `/`"那一跳**带不上刚拿到的 cookie**，`/` 就 401 了。地址栏闪过 token 恰恰证明 **token 是被接受的**（那是一次 303）。手动粘贴或按 F5 都是**同站导航**，cookie 就带上了，所以能进。
- ✅ **要一次到位，走配对路径**：在「实例」面板里用那台机器**「远程访问」面板生成的配对链接**把同一个地址配上。之后面板会自动改用 `pair-app?device=…` 的**免 token 落地页**（代码里配对凭据的优先级高于 token），点一次就进、不需要刷新。原因是那条路径的中间页**由远端自己提供**，它再跳 `/` 就是同站的了 —— 这也正是它不受 `SameSite=Strict` 影响的原因。
  > ⚠️ 别指望用**启动 token** 去自动换配对凭据：`/api/pair/accept` 要的是配对链接里的 `?pair=` 令牌，和启动 token（`?token=`）是两回事（`parsePairingLink` 只认 `pair` 与 `device` 两个参数）。
