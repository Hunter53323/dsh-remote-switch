/**
 * dsh-instance-switcher — browser half.
 *
 * One sidebar footer entry beside Settings: a dropdown that switches this
 * browser to another DSH instance's official Web GUI, a list to manage the
 * stored instances, and a highlight for the instance you are currently in.
 *
 ...
 *
 * Switching is one navigation: `<origin>/pair-app?device=<credential>` opens
 * that instance's official shell without depending on any cookie this browser
 * holds. "本机" is a fixed `http://127.0.0.1:<port>` with no credential, so it
 * only works while sitting at that machine — which is exactly what "local"
 * means. Nothing here talks to another instance itself: every HTTP call goes
 * to this instance's own `/api/instance-switcher/*`.
 */
window.__ModuleLoader__.load({
	id: 'dsh-instance-switcher',
	factory: (require) => {
		var module = { exports: {} }
