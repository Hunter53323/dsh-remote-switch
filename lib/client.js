...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ZH = {
			...
			'fed.tokenJumpNeedsReload': '这台机器用 token 跳转：打开后如果停在「authentication required」，按一次刷新（F5）就进去了 —— harness 的登录 cookie 是 SameSite=Strict，跨站打开时第一跳带不上它，而刷新是同站导航。进去之后可以直接收藏不带 token 的地址。',
			'fed.current': '当前',
		...
		const EN = {
			...
			'fed.tokenJumpNeedsReload': 'This machine jumps by token: if it stops on "authentication required", press reload (F5) once and you are in — the harness\'s login cookie is SameSite=Strict, so a cross-site open cannot carry it on the first hop, while a reload is a same-site navigation. Once inside, the token-less address is worth bookmarking.',
			'fed.current': 'active',
