...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ZH = {
			...
			'fed.tokenJumpNeedsReload': '这台机器靠 token 跳转。harness 的登录 cookie 是 SameSite=Strict，而这里是从本面板（另一个源）发起的跨站打开，那一跳带不上 cookie，所以会停在登录页 —— token 本身没问题（地址栏闪一下就是它被接受了）。按 F5 能进，但**这个按钮每次都需要这一步**。要一次到位：在「实例」面板用那台机器的配对链接配上同一地址，之后这里会自动改用免 token 的落地页，点一次就进。',
			'fed.current': '当前',
			...
			'editor.provisionLogs': '查看远端日志',
			'editor.provisionLogsEmpty': '远端日志是空的或不存在（只有本插件拉起过那台实例才会有这份日志）。',
		...
		const EN = {
			...
			'fed.tokenJumpNeedsReload': 'This machine jumps by token. The harness login cookie is SameSite=Strict and this is a cross-site open from the panel, so that hop cannot carry it and you land on the login page — the token itself is fine (the address bar flashing it is the acceptance). F5 gets you in, but this button needs that EVERY time. For one click: pair the same address in the "Instances" panel with that machine\'s pairing link, and this switches to the token-less landing page.',
			'fed.current': 'active',
			...
			'editor.provisionLogs': 'Show the remote log',
			'editor.provisionLogsEmpty': 'The remote log is empty or missing (it exists only once this plugin has started that instance).',
		...
		function FederationPanel() {
			...
			return h('div', {
				...
				h('div', { key: 'machines', style: { flex: 'none', borderTop: `1px solid ${T.border}`, padding: '8px 14px 10px' } }, [
					...
					// A token jump cannot complete on its first hop, and the reason is
					// not fixable from here: the harness's browser-session cookie is
					// `SameSite=Strict`, while this button opens the remote from a
					// DIFFERENT origin — a cross-site-initiated navigation, in which the
					// browser will not attach that Strict cookie. So the redeem 303 to `/`
					// arrives without the cookie it just minted, and the remote answers
					// 401. A reload is same-site, so the cookie rides it. Say so, because
					// the symptom (address bar flashing the token, then a bare origin and
					// a login page) looks like the token was rejected — it was accepted.
					typeof active?.jumpUrl === 'string' && active.jumpUrl.includes('?token=')
						? h('div', {
							key: 'tokenJump',
							style: { fontSize: 10, color: T.muted, marginTop: 4, lineHeight: 1.5 },
						}, t('fed.tokenJumpNeedsReload'))
						: null,
					editing !== undefined
		...
		function PeerEditor(props) {
			...
			const seedForm = (peer) => ({
				...
				// Prefilled: the stored secrets are shown rather than withheld (the
				// owner's call), so editing a machine starts from what is saved.
			...
			const provision = async (action, options = {}) => {				if (editing === undefined) return
				...
				try {
					...
					if (action === 'logs') {
						// The user's own debugging view: what the remote actually
						// printed. The host masks `token=` and leaves the ports and
						// addresses, because "which port did it come up on" is the
						// question this answers.
						const log = typeof outcome.log === 'string' ? outcome.log.trim() : ''
						setMessage(log === ''
							? `${t('editor.provisionLogs')}\n${outcome.detail ?? t('editor.provisionLogsEmpty')}`
							: `${t('editor.provisionLogs')}\n${log.slice(-3000)}`)
						return
					}
					if (action === 'stop') {
			...
			return h('div', { style: { marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 560 } }, [
				...
				channel === 'ssh'
					? h('div', { key: 'ssh', style: { display: 'flex', flexDirection: 'column', gap: 8 } }, [
						...
						// Shown in the clear, along with the token below: the owner asked
						// for their own stored credentials to be visible in their own panel
						// instead of withheld. So the box is prefilled from the peer (see
						// `seedForm`) and the label is the ordinary one — the "saved, leave
						// empty" wording existed only because the field used to be empty.
						field('password', t('editor.password'), t('editor.passwordNew'), 'password'),
				...
				h('div', { key: 'auth', style: { display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, color: T.muted } }, [
					...
					// Same as the SSH password above: prefilled and shown, not hidden.
					form.authKind === 'token'
						? field('token', t('editor.token'), t('editor.tokenPlaceholder'))
						: null,
				...
				editing !== undefined && editing.channel === 'ssh'
					? h('div', { key: 'prov', style: { borderTop: `1px solid ${T.border}`, paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 6 } }, [
						...
						h('div', { key: 'row', style: { display: 'flex', gap: 6, flexWrap: 'wrap' } }, [
							...
							h('button', { key: 'g', type: 'button', disabled: busy, onClick: () => { void provision('logs') }, style: button }, t('editor.provisionLogs')),
						]),
					])
					: null,
