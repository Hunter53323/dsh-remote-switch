...
/**
 * Whether a candidate token is accepted by the remote *right now*.
 *
 * The token is per-boot (measured: the previous boot's token answers `GET
 * /?token=` with 401 while the current one answers 303), so a token scraped out
 * of a log can be a dead one — the log is only written when this plugin starts
 * the instance, and it is only cleared at that moment. Redeeming a candidate
 * before trusting it is what separates "the log says there was a token" from
 * "this is the token that instance is serving today".
 *
 * Deliberately does NOT go through `resolveCarrier`: resolving a carrier for a
 * token peer redeems the peer's *stored* credential first, so it would validate
 * the wrong token (and fail outright once that one is stale).
 *
 * On success the minted cookie is cached for the peer, because the validated
 * token is by definition the one worth keeping.
 *
 * @param {object} peer - the peer row (its `origin` and channel are what matter).
 * @param {string} token - the candidate token.
 * @param {{ transport?: import('./ssh.js').SshTransport, auth: import('./auth.js').AuthStore, timeoutMs?: number }} deps - runtime dependencies.
 * @returns {Promise<{ ok: true } | { ok: false, code: string, detail: string }>} the verdict.
 */
export async function verifyToken(peer, token, deps) {
  const timeoutMs = deps.timeoutMs ?? REQUEST_TIMEOUT_MS
  const createConnection = peer.channel === 'ssh' && deps.transport !== undefined
    ? deps.transport.createConnection(peer)
    : undefined
  const request = url => httpRequest(url, { createConnection, timeoutMs })
  const candidate = { ...peer, auth: { kind: 'token', token } }
  try {
    await deps.auth.redeem(candidate, request)
    return { ok: true }
  } catch (error) {
    return {
      ok: false,
