...
/**
 * Project one stored peer row into the shape the panel reads.
 *
 * **The stored secrets are included in the clear** — the token and the SSH
 * password are sent as values, not as presence flags. That is the owner's
 * explicit decision for this plugin: they want to see what their own panel
 * holds. (It reverses the earlier rule, which sent only `hasToken` /
 * `hasPassword`; the jump URL still embeds a credential because that is how DSH
 * authenticates a browser, so that part is unchanged in kind.) The consequence
 * to keep in mind is not the network — these routes are loopback-fenced — it is
 * being seen: screenshots, screen sharing, and the browser history of a jump.
 * The private-key *path* was always shown, since a path is not the key.
 *
 * `jumpUrl` is still built here rather than in the browser, so the panel gets a
 * ready URL instead of reassembling one. Two credentials can become that URL, in
 * order of preference:
 *
...
export function redactPeer(peer, deviceCredentialOf) {
  const view = {
    ...
    origin: peer.origin,
    auth: {
      kind: peer.auth?.kind ?? 'none',
      hasToken: typeof peer.auth?.token === 'string',
      // The token is handed over on purpose. The owner of this plugin asked to
      // see the credentials their own panel holds rather than have them withheld
      // — which is a deliberate reversal of the earlier "presence flag only"
      // rule, not an oversight. Consequences worth knowing: the panel is
      // loopback-fenced, so the exposure is not the network, it is *being seen*
      // (screenshots, screen sharing, browser history for the jump URL).
      ...(typeof peer.auth?.token === 'string' ? { token: peer.auth.token } : {}),
    },
  ...
  if (peer.ssh !== undefined) {
    view.ssh = {
      ...
      // Shown for the same reason as the token above.
      ...(typeof peer.ssh.password === 'string' ? { password: peer.ssh.password } : {}),
    }
...
export function applyFederation(ctx, config) {
   ...
   *
   * An allow-list rather than a spread: `start()` returns a lot of internally
   * shaped fields, and the panel needs a known handful of them. This is about
   * keeping the response shape explicit, NOT about withholding the token — the
   * peer list shows stored credentials in the clear by the owner's decision.
   * @param {object} result - the provisioner's return value.
   * @returns {object} the fields the panel actually reads.
   */
  const publicResult = result => {
    ...
    // `detail` is free text derived from remote output, and the raw log tail is
    // shown as-is too (the owner asked to see every credential rather than have
    // any of it withheld), so nothing is masked on the way out.
  ...
  const handleProvision = async (req, res) => {
    ...
    try {
      ...
      if (action === 'logs') {
        // The remote's own log, on demand. It is the only place the answers to
        // "which port did it actually come up on" and "why did it not come up"
        // exist, and until now the panel only ever showed it as part of a FAILED
        // start — useless when the start reports success but the browser still
        // cannot get in. The token is masked; the ports and addresses stay, since
        // comparing them against the configured jump address is the whole point.
        const log = await provisioner.tailLog(peer, remoteHome, 200)
        const empty = log.trim() === ''
        sendJson(res, 200, {
          ...state(),
          provision: {
            id: peer.id,
            ...
            empty,
            ...(empty ? {
              detail: '远端那份日志是空的或不存在：这台实例可能不是本插件拉起的（日志在对方 <DSH_HOME>/federation/web.log，只有本插件拉起时才会写）',
            } : {}),
          },
        })
        return
      }
      sendJson(res, 400, { ok: false, error: 'unknown-action', hint: 'action 只能是 start / stop / status / read-token / logs' })
    } catch (error) {
      sendJson(res, 200, {
        ...state(),
        provision: {
