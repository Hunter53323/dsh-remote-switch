...
import { probePeer, readPeer } from './client.js'
 ...
 * paired-device credential that makes a cookieless jump work never leaves this
 * process. The browser receives a ready URL and cannot read the secret out of
 * it any more than it could read any other URL it is handed.
 *
...
export function redactPeer(peer, deviceCredentialOf) {
  ...
  // origin) can be opened, and only when a credential exists to open it with.
  const browserOrigin = browserOriginOf(peer)
  if (browserOrigin !== undefined) {
    const credential = deviceCredentialOf(browserOrigin)
    if (credential !== undefined) view.jumpUrl = entryUrl(browserOrigin, credential)
    else view.openOrigin = browserOrigin
  }
  return view
}
