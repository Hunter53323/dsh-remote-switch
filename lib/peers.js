/**
 * Peer-instance store for dsh-remote-switch.
 *
 ...
 *
 * Only stored peers live here. The entry for the instance you are already in
 * is deliberately absent: the panel belongs to that instance, so a local row
 * would point back at the same origin. The legacy `__local__` id is still
 * refused on load so an old file cannot smuggle one back in.
 *
 * @module dsh-remote-switch/peers
 */
